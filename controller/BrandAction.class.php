...
class BrandAction extends  Action
{
    ...
    public  function  index(){
        ...
        $this->_tpl->display(SMARTY_ADMIN.'brand/show.html');
    }

    public  function  add(){
        if (isset($_POST['send'])) $this->_model->add() ? $this->_redirect->succ('?a=brand', '品牌新增成功！') : $this->_redirect->error('品牌新增失败！');
        $this->_tpl->display(SMARTY_ADMIN . 'brand/add.html');
    }
    public function update(){
        if (isset($_POST['send'])) $this->_model->update() ? $this->_redirect->succ('?a=brand', '品牌修改成功！') : $this->_redirect->error('品牌修改失败！');
        if (isset($_GET['id'])) {
            $this->_tpl->assign('OneBrand', $this->_model->findOne());
            $this->_tpl->display(SMARTY_ADMIN.'brand/update.html');
        }
        }
